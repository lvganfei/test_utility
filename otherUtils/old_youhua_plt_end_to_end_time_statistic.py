...
push_sql_uri = 'mysql+pymysql://root:qY1WBZ30vjsriiHiEgK2ZkE8osjYNsj@10.12.10.113:13306/universe_test?charset=utf8mb4'
start_time_flag = ''
today = ''
...
engine1 = create_engine(push_sql_uri,pool_recycle=3600, echo=False)
...
for record in result:
    ...
    if record[3] == 'FROM_PUSH':
        ...
        PUSH_RECORD_STATEMENT = f'select start_time, finish_time from universe_test.push_statistics where series_instance_uid="{series_instance_uid}"'
        
        series_push_result = session.execute(PUSH_RECORD_STATEMENT).fetchone()
        push_start_time = series_push_result[0]
        push_end_time = series_push_result[1]
    ...
    #如果是主动拉取的数据
    elif record[3] == 'FROM_MOVE':
        series_instance_uid = record[0]
        DICOM_SERIES_FIND_SQL_STATEMENT = f'SELECT series_instance_uid,series_datetime,created_at,s_move_time,e_move_time FROM plt_dicom.dicom_series_find where series_instance_uid="{series_instance_uid}"'
        series_find_result = session.execute(DICOM_SERIES_FIND_SQL_STATEMENT).fetchone()
        find_series_time = series_find_result[2]
        move_start_time = series_find_result[3]
        move_end_time = series_find_result[4]
    ...
    tmp_arr = list(record)
    tmp_arr = tmp_arr[:5] + [push_start_time, receive_start_time, push_end_time, receive_end_time, find_series_time, move_start_time, move_end_time] + tmp_arr[5:]
    
    origin_arr.append(tmp_arr)

session.close()
df = pd.DataFrame(origin_arr, columns=["series_instance_uid","case_num","study_datetime","series_receive_way","series_datetime","push_start_time","receive_start_time","push_end_time","receive_end_time", "find_series_time", "move_start_time", "move_end_time","series_create_time","case_create_time","alg_start_at","alg_finish_at","case_finish_time"])
df.to_csv(f'./old_1.12.1_plt_e2e_time_statistic_{today}.csv')
