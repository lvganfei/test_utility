...
def run():
    total_arr = []
    retrieve_days = int(sys.argv[1])
    current = datetime.datetime.now()
    for n in range(retrieve_days):
        if n == 0:
            continue
        today = current - datetime.timedelta(days=n)
        tmp_arr = get_today_result(today)
        total_arr.extend(tmp_arr)

    print(total_arr)
    with open(f'./sk_e2e_time_statistic_cn.csv','w') as result_csv:
...
def get_today_result(today):
    formated_today = today.strftime('%Y-%m-%d')
    start_time_flag = formated_today + ' 00:00:00'
    end_time_flag = formated_today + ' 23:50:00'
