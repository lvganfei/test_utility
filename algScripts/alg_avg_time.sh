...
count=0
today=`date +'%Y-%m-%d'`
echo "case_num,version,total_time" >> alg_time_all_$1_$today.csv
...
do
  ...
  then
    ...
    ltime=`cat "$folder/$i/$2.log" | grep 'All Completed' | awk '{print $4}'`
    if [[ -n "$ltime" ]] && [[ -n "$version" ]];
    then
  
      echo -ne "\n$i,$version,$ltime" >> alg_time_all_$1_$today.csv
      oldifs="$IFS"
      IFS=$'\n'
      for component in `cat "$folder/$i/$2.log" | grep 'Finished' | awk -F 'Finished ' '{print $2}' | awk -F ' in ' '{print $1 $2}' | awk -F ' s. ' '{print $1}'`
        do
          ...
          echo -n ",$component_time" >> alg_time_all_$1_$today.csv
        done
      IFS="$oldifs"
...
echo "average,$avg" >> alg_time_all_$1_$today.csv