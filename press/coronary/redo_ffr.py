...
host = f'http://ffr.{env}.platform.democompany.net'
...
def redo(case):
    print(case)
    print(requests.post(url=f'{host}/api/ffr/case/{case}/repostprocess',
                        json={},
                        headers=auth).text)
...
def get_csv_case():
    data = pd.read_excel(io='./0.99.5.cd0e6541冠脉算法各项合格率统计文档.xlsx',
                         sheet_name='结果记录表',
                         usecols='B')
...
if __name__ == '__main__':
    session = MysqlSession(f'mysql+pymysql://root:qY1WBZ30vjsriiHiEgK2ZkE8osjYNsj@109.244.38.201:'
                           f'{env_mysql_port}/plt_ffr?charset=utf8mb4')
